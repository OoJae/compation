...
- [ ] Tests green: 59 agent unit tests pass (risk engine 33, decimal scaling 13, what-if 5, others).
...
**What's innovative**
> Compation is the first AI agent on the on-chain H100 compute-derivative market, and it assembles three things that have never been combined before: (1) automated H100 hedging — turning a trader-only perp into a one-sentence action for an AI founder; (2) an ERC-8004 on-chain identity (NFT #49) that makes the agent a registered economic actor earning protocol fee rebates as feeRecipient; and (3) x402 self-payment — the agent pays $0.01 USDC on-chain for the compute it uses. It's not a demo; it's a registered economic actor with a built-in business model. And the trust model is explicit: the language model reasons, a deterministic risk engine sizes — Compation never hallucinates a position size, every number is computed and constraint-checked, and every decision is logged.
